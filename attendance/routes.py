...
async def list_today_attendance(db: Session = Depends(get_db)):
    return db.query(attendance.Attendance).filter(attendance.Attendance.clock_in_time >= datetime.datetime.now().replace(hour=0, minute=0, second=0)).all() 

@router.post("/attend")
async def create_attendance(employee_id: int, db: Session = Depends(get_db)):
    attendance_record = attendance.Attendance(employee_id=employee_id, clock_in_time=datetime.datetime.now())
...
async def update_attendance(employee_id: int, db: Session = Depends(get_db)):
    ...
    attendance_record = db.query(attendance.Attendance).filter(attendance.Attendance.employee_id == employee_id, attendance.Attendance.clock_out_time == None, attendance.Attendance.clock_in_time >= datetime.datetime.now().replace(hour=0, minute=0, second=0)).first()
